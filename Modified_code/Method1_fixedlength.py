...
def Extract_segmented_trials(emg, stimulus, fs, trial_duration=5, break_label=0):
...
def Leave_one_trial(trials, labels, k_range):
    n_trials = len(trials)
...
def Plotting_k(k_scores, channel):
    k_values = list(k_scores.keys())
    mean_scores = [np.mean(scores) for scores in k_scores.values()]
...
def Best_possible_accuracy(emg, stimulus, fs, k_range=range(1, 20)):
    cutoff_frequency = 10  
    ...
    for channel in range(num_channels):
        ...
        trials, labels = Extract_segmented_trials(emg_filtered[:, channel].reshape(-1, 1), stimulus, fs)

        k_scores = Leave_one_trial(trials, labels, k_range)
        max_k, max_acc = Best_K_accuracy(k_scores)
...
def main():
    ...
    subject, emg, stimulus = Loading_files(file_path)

    best_channel, best_k, best_accuracy, all_results = Best_possible_accuracy(emg, stimulus, fs)

    trials, labels = Extract_segmented_trials(emg, stimulus, fs, trial_duration, break_label)
