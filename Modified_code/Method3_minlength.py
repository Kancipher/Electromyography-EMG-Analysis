...
def extract_combined(trial, feature_type='combined'):
...
def segment_trials(emg, stimulus, fs):
...
def leave_one_trial_out_cv_combined(trials, labels, k_range, feature_type='combined'):
    ...
    X = np.array([extract_combined(trial, feature_type) for trial in trials])
    y = np.array(labels)
...
def find_best_combined_configuration(emg, stimulus, fs, k_range=range(1, 20), duration_time=5):
    ...
    trials, labels = segment_trials(filtered_data, stimulus, fs)

    k_scores_time = leave_one_trial_out_cv_combined(trials, labels, k_range, feature_type='time')
    ...
    k_scores_freq = leave_one_trial_out_cv_combined(trials, labels, k_range, feature_type='frequency')
    ...
    plot_k(k_scores_freq, "Combined time and frequency Features")
