...
def extract_frequency(signal, minlength):
    
    segment=signal[:minlength]
    
    fft_values = np.fft.fft(segment, axis=0)
...
def leave_one_trial_out_cv(trials, labels, k_range, fs):
    ...
    X = np.array([extract_frequency(trial, min_length) for trial in trials])
    y = np.array(labels)
...
def find_best_configuration(emg, stimulus, fs, k_range=range(1, 21)):
    best_accuracy = 0
    best_channel = None
    best_k = None
...
def main():
    fs = 100
    file_path = "C://Users//Omar Ganna//Desktop//Project-1//subject_2.mat"
